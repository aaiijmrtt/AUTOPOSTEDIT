	...
	with tf.name_scope('encoder'):
		...
		for i in xrange(dim_d):
			...
			with tf.name_scope('inputgate_%i' %i):
				model['eWi_%i' %i] = tf.Variable(tf.random_uniform([dim_i, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), collections = [tf.GraphKeys.VARIABLES, tf.GraphKeys.REGULARIZATION_LOSSES], name = 'eWi_%i' %i)
				model['eBi_%i' %i] = tf.Variable(tf.random_uniform([1, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), name = 'eBi_%i' %i)
				for ii in xrange(dim_t):
			...
			with tf.name_scope('forgetgate_%i' %i):
				model['eWf_%i' %i] = tf.Variable(tf.random_uniform([dim_i, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), collections = [tf.GraphKeys.VARIABLES, tf.GraphKeys.REGULARIZATION_LOSSES], name = 'eWf_%i' %i)
				model['eBf_%i' %i] = tf.Variable(tf.random_uniform([1, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), name = 'eBf_%i' %i)
				for ii in xrange(dim_t):
			...
			with tf.name_scope('cellstate_%i' %i):
				model['eWc_%i' %i] = tf.Variable(tf.random_uniform([dim_i, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), collections = [tf.GraphKeys.VARIABLES, tf.GraphKeys.REGULARIZATION_LOSSES], name = 'eWc_' + str(i))
				model['eBc_%i' %i] = tf.Variable(tf.random_uniform([1, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), name = 'eBc_' + str(i))
				for ii in xrange(dim_t):
					model['ecc_%i_%i' %(i, ii)] = tf.Variable(tf.random_uniform([dim_b, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), name = 'ecc_%i_%i' %(i, ii)) if ii == 0 else model['ec_%i_%i' %(i, ii - 1)] # consider starting with all zeros
			...
			with tf.name_scope('hidden_%i' %i):
				model['eWz_%i' %i] = tf.Variable(tf.random_uniform([dim_i, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), collections = [tf.GraphKeys.VARIABLES, tf.GraphKeys.REGULARIZATION_LOSSES], name = 'eWz_%i' %i)
				model['eBz_%i' %i] = tf.Variable(tf.random_uniform([1, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), name = 'eBz_%i' %i)
				for ii in xrange(dim_t):
