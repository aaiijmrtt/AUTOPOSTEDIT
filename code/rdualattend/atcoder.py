	...
	with tf.name_scope(scope):
		...
		for i in xrange(dim_d):
			...
			with tf.name_scope('attention_%i' %i):
				model['dWa1_%i' %i] = tf.Variable(tf.truncated_normal([dim_i, dim_i], stddev = 1.0 / dim_i), collections = [tf.GraphKeys.REGULARIZATION_LOSSES], name = 'dWa1_%i' %i)
				model['dBa1_%i' %i] = tf.Variable(tf.truncated_normal([dim_m, 1], stddev = 1.0 / dim_t), name = 'dBa1_%i' %i)
				model['dWa2_%i' %i] = tf.Variable(tf.truncated_normal([dim_i, dim_i], stddev = 1.0 / dim_i), collections = [tf.GraphKeys.REGULARIZATION_LOSSES], name = 'dWa2_%i' %i)
