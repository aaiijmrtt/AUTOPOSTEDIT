	...
	with tf.name_scope('bicoder'):
		...
		for i in xrange(dim_d):
			...
			with tf.name_scope('inputgate_%i' %i):
				model['eFWi_%i' %i] = tf.Variable(tf.random_uniform([dim_i, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), collections = [tf.GraphKeys.VARIABLES, tf.GraphKeys.REGULARIZATION_LOSSES], name = 'eFWi_%i' %i)
				model['eFBi_%i' %i] = tf.Variable(tf.random_uniform([1, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), name = 'eFBi_%i' %i)
				model['eBWi_%i' %i] = tf.Variable(tf.random_uniform([dim_i, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), collections = [tf.GraphKeys.VARIABLES, tf.GraphKeys.REGULARIZATION_LOSSES], name = 'eBWi_%i' %i)
				model['eBBi_%i' %i] = tf.Variable(tf.random_uniform([1, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), name = 'eBBi_%i' %i)
				for ii in xrange(dim_t):
			...
			with tf.name_scope('forgetgate_%i' %i):
				model['eFWf_%i' %i] = tf.Variable(tf.random_uniform([dim_i, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), collections = [tf.GraphKeys.VARIABLES, tf.GraphKeys.REGULARIZATION_LOSSES], name = 'eFWf_%i' %i)
				model['eFBf_%i' %i] = tf.Variable(tf.random_uniform([1, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), name = 'eFBf_%i' %i)
				model['eBWf_%i' %i] = tf.Variable(tf.random_uniform([dim_i, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), collections = [tf.GraphKeys.VARIABLES, tf.GraphKeys.REGULARIZATION_LOSSES], name = 'eBWf_%i' %i)
				model['eBBf_%i' %i] = tf.Variable(tf.random_uniform([1, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), name = 'eBBf_%i' %i)
				for ii in xrange(dim_t):
			...
			with tf.name_scope('outputgate_%i' %i):
				model['eWo_%i' %i] = tf.Variable(tf.random_uniform([dim_i, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), collections = [tf.GraphKeys.VARIABLES, tf.GraphKeys.REGULARIZATION_LOSSES], name = 'eWo_%i' %i)
				model['eBo_%i' %i] = tf.Variable(tf.random_uniform([1, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), name = 'eBo_%i' %i)
				for ii in xrange(dim_t):
			...
			with tf.name_scope('cellstate_%i' %i):
				model['eFWc_%i' %i] = tf.Variable(tf.random_uniform([dim_i, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), collections = [tf.GraphKeys.VARIABLES, tf.GraphKeys.REGULARIZATION_LOSSES], name = 'eFWc_' + str(i))
				model['eFBc_%i' %i] = tf.Variable(tf.random_uniform([1, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), name = 'eFBc_' + str(i))
				model['eBWc_%i' %i] = tf.Variable(tf.random_uniform([dim_i, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), collections = [tf.GraphKeys.VARIABLES, tf.GraphKeys.REGULARIZATION_LOSSES], name = 'eBWc_' + str(i))
				model['eBBc_%i' %i] = tf.Variable(tf.random_uniform([1, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), name = 'eBBc_' + str(i))
				for ii in xrange(dim_t):
					model['eFcc_%i_%i' %(i, ii)] = tf.Variable(tf.random_uniform([dim_b, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), name = 'eFcc_%i_%i' %(i, ii)) if ii == 0 else model['eFc_%i_%i' %(i, ii - 1)] # consider starting with all zeros
				...
				for ii in reversed(xrange(dim_t)):
					model['eBcc_%i_%i' %(i, ii)] = tf.Variable(tf.random_uniform([dim_b, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), name = 'eBcc_%i_%i' %(i, ii)) if ii == dim_t - 1 else model['eBc_%i_%i' %(i, ii + 1)] # consider starting with all zeros
			...
			with tf.name_scope('hidden_%i' %i):
				model['eWz_%i' %i] = tf.Variable(tf.random_uniform([2 * dim_i, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), collections = [tf.GraphKeys.VARIABLES, tf.GraphKeys.REGULARIZATION_LOSSES], name = 'eFWz_%i' %i)
				model['eBz_%i' %i] = tf.Variable(tf.random_uniform([1, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), name = 'eFBz_%i' %i)
				for ii in xrange(dim_t):
		...
		with tf.name_scope('negativeloglikelihood'):
			for ii in xrange(dim_t):
				model['enll_%i' %ii] = tf.select(tf.equal(model['exi_%i' %ii], tf.zeros([dim_b], tf.int32)), tf.zeros([dim_b], tf.float32), tf.nn.sampled_softmax_loss(model['We'], tf.zeros([dim_v], tf.float32), model['eh_%i' %ii], tf.reshape(model['eyi_%i' %ii], [dim_b, 1]), samp, dim_v), name = 'enll_%i' %ii)
