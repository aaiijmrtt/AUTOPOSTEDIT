	...
	with tf.name_scope('decoder'):
		...
		for i in xrange(dim_d):
			...
			if biencoder:
				with tf.name_scope('transferstate_%i' %i):
					model['dWt_%i' %i] = tf.Variable(tf.truncated_normal([2 * dim_i, dim_i], stddev = 1.0 / dim_i), name = 'dWt_%i' %i)
		...
		with tf.name_scope('meansquared'):
			for ii in xrange(dim_t):
				...
				for i in xrange(dim_d):
					model['dmsa_%i_%i' %(i, ii)] = tf.select(tf.equal(model['dai_%i_%i' %(i, ii)], -tf.ones([dim_m, dim_b], tf.float32)), tf.zeros([dim_m, dim_b], tf.float32), tf.square(tf.sub(model['dai_%i_%i' %(i, ii)], model['dat_%i_%i' %(i, ii)])), name = 'dmsa_%i_%i' %(i, ii))
			model['dmss'] = tf.reduce_sum(tf.add_n([model['dmss_%i' %ii] for ii in xrange(dim_t)]), name = 'dmss')
			model['dmsa'] = tf.reduce_sum(tf.add_n([model['dmsa_%i_%i' %(i, ii)] for ii in xrange(dim_m) for i in xrange(dim_d)]), name = 'dmsa')
