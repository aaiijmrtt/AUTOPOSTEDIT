...
def create(embedder, encoder1, encoder2, combiner, config, scope = 'fatcoder'):
	...
	with tf.name_scope(scope):
		...
		with tf.name_scope('attention'):
			for ii in xrange(dim_t):
				model['dai1_%i' %ii] = tf.placeholder(tf.float32, [dim_t, dim_b], name = 'dai1_%i' %ii)
				model['dai2_%i' %ii] = tf.placeholder(tf.float32, [dim_t, dim_b], name = 'dai2_%i' %ii)
		...
		with tf.name_scope('meansquared'):
			for ii in xrange(dim_t):
				...
				for i in xrange(dim_d):
					model['dmsa1_%i_%i' %(i, ii)] = tf.select(tf.equal(model['dai1_%i_%i' %(i, ii)], -tf.ones([dim_t, dim_b], tf.float32)), tf.zeros([dim_t, dim_b], tf.float32), tf.square(tf.sub(model['dai1_%i_%i' %(i, ii)], model['dat1_%i_%i' %(i, ii)])), name = 'dmsa1_%i_%i' %(i, ii))
					model['dmsa2_%i_%i' %(i, ii)] = tf.select(tf.equal(model['dai2_%i_%i' %(i, ii)], -tf.ones([dim_t, dim_b], tf.float32)), tf.zeros([dim_t, dim_b], tf.float32), tf.square(tf.sub(model['dai2_%i_%i' %(i, ii)], model['dat2_%i_%i' %(i, ii)])), name = 'dmsa2_%i_%i' %(i, ii))
			model['dmss'] = tf.reduce_sum(tf.add_n([model['dmss_%i' %ii] for ii in xrange(dim_t)]), name = 'dmss')
			model['dmsa'] = tf.add(tf.reduce_sum(tf.add_n([model['dmsa1_%i_%i' %(i, ii)] for ii in xrange(dim_t) for i in xrange(dim_d)])), tf.reduce_sum(tf.add_n([model['dmsa1_%i_%i' %(i, ii)] for ii in xrange(dim_t) for i in xrange(dim_d)])), name = 'dmsa')
