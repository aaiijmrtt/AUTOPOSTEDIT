	...
	with tf.name_scope(scope):
		for i in xrange(dim_d):
			with tf.name_scope('layer_%i' %i):
				model['cW_%i' %i] = tf.Variable(tf.truncated_normal([2 * dim_i, dim_i], stddev = 1.0 / dim_i), name = 'cW_%i' %i) if i == 0 else tf.Variable(tf.truncated_normal([dim_i, dim_i], stddev = 1.0 / dim_i), name = 'cW_%i' %i)
