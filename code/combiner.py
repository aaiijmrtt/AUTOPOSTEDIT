...
def create(encoder1, encoder2, config, scope = 'combiner'):
	dim_i, dim_d, dim_t, dim_b, dim_k, nonlinear = config.getint('wvec'), config.getint('depth'), config.getint('_steps_'), config.getint('batch'), config.getint('kvec'), getattr(tf.nn, config.get('nonlinear'))
	model = dict()

	with tf.name_scope(scope):
		for i in xrange(dim_d):
			with tf.name_scope('layer_%i' %i):
				model['cW_%i' %i] = tf.Variable(tf.truncated_normal([dim_i + dim_k, dim_i], stddev = 1.0 / dim_i), name = 'cW_%i' %i) if i == 0 else tf.Variable(tf.truncated_normal([dim_i, dim_i], stddev = 1.0 / dim_i), name = 'cW_%i' %i)
