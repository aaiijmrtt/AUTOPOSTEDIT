	...
	with tf.name_scope(scope):
		for i in xrange(dim_d):
			with tf.name_scope('layer_%i' %i):
				model['cW_%i' %i] = tf.Variable(tf.random_uniform([2 * dim_i, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), name = 'cW_%i' %i) if i == 0 else tf.Variable(tf.random_uniform([dim_i, dim_i], - np.sqrt(6. / dim_i), np.sqrt(6. / dim_i)), collections = [tf.GraphKeys.VARIABLES, tf.GraphKeys.REGULARIZATION_LOSSES], name = 'cW_%i' %i)
